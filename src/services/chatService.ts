...
class ChatService {
  ...
  async getVibeSessions(userId?: string, anonymousUserId?: string, pagination?: VibeSessionParams): Promise<VibeSessionsResponse> {
    try {
      const sessions = await this.chatApi.getVibeSessions(userId, anonymousUserId, pagination);
      return sessions;
    } catch (error) {
