...
class PersonaSessionManager {
	 ...
	 */
	private setStoredPersonaUser(personaId: Persona['id'], user: StoredPersonaUser | null): void {
		const storedUsers = this.getStoredPersonaUsers();
	...
	setUserId(personaId: Persona['id'], userId: string): void {
		...
		if (activeSession?.personaId === personaId) {
			this.updateActivePersonaSession?.({
				userId,
				userInfo: activeSession.userInfo ? { ...activeSession.userInfo, id: userId } : null,
			});
		}
	 ...
	 */
	onSessionChange(personaId: Persona['id'], callback: SessionChangeCallback): () => void {
