...
export class SignalRService {
	...
	public createConnection(): void {
		...
		if (scheduleChangeHub) {
			scheduleChangeHub.client.refereshDataFromSockets =
				this.onScheduleChangeReceipt.bind(this);
		} else {
		...
		window.$.connection.hub.disconnected(() => {
			...
			setTimeout(() => {
				if (
					window.$.connection.hub.state === window.$.signalR.connectionState.disconnected
				) {
	...
	public sendToServer(methodName: string, ...args: unknown[]): Promise<unknown> | undefined {
		if (
			...
			this.signalRConnection.state === window.$.signalR.connectionState.connected
		) {
