...
export function usePersonaSessionManager() {
	...
	return {
		...
		setUserId: useCallback(
			(personaId: Persona['id'], userId: string) => manager.setUserId(personaId, userId),
			[manager]
		),
		...
		clearDevOverride: useCallback(() => manager.clearDevOverride(), [manager]),
		isDevOverrideActive: useCallback(() => manager.isDevOverrideActive(), [manager]),
		...
		onAnySessionChange: useCallback(
			(callback: SessionChangeCallback) => manager.onAnySessionChange(callback),
			[manager]
		),

		// Utility methods
		clearExpiredUsers: useCallback(() => manager.clearExpiredUsers(), [manager]),
		getAllStoredUsers: useCallback(() => manager.getAllStoredUsers(), [manager]),
	};
}
