...
const SessionHistory: React.FC<SessionHistoryProps> = ({
	...
	const fetchSessions = useCallback(
		async (index: number, append: boolean = false) => {
			if (!userId) return;
						...
						<>
							{sessions.map((session) => (
								...
								>
									<SessionTitle>
										{session.title ||
											t('home.expanded.chat.sessionHistory.untitled')}
									</SessionTitle>
									<SessionTimestamp>
								...
								<LoadMoreButton onClick={handleLoadMore} disabled={isLoadingMore}>
									{isLoadingMore ? (
										<>
											...
											<span>
												{t('home.expanded.chat.sessionHistory.loadingMore')}
											</span>
										</>
									) : (
										<span>
											{t('home.expanded.chat.sessionHistory.loadMore')}
										</span>
									)}
								</LoadMoreButton>
							)}
...
const SessionItem = styled.button<{ $isActive: boolean }>`
	...
	border-left: 2px solid
		${({ $isActive, theme }) => ($isActive ? theme.colors.brand[500] : 'transparent')};
