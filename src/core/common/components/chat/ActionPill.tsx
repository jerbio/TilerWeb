...
const ActionPill: React.FC<ActionPillProps> = ({ action }) => {
  ...
  /** Show status overlay for a brief duration */
  const showOverlay = useCallback((message: string, variant: StatusOverlayVariant = 'info', loading = false) => {
  ...
  const handleClick = () => {
    ...
    if (isStale) {
      showOverlay(t('home.expanded.chat.tileStale', 'This tile may have changed since a newer update was accepted'), 'warning');
      return;
    }

    // Pending actions (pre-accept) — tile doesn't exist on calendar yet
    if (isPending) {
      showOverlay(t('home.expanded.chat.acceptToSee', 'Accept changes to see this tile'), 'info');
      return;
    }
    ...
    calendarDispatch(
      ...
      (result: CalendarRequestResult) => {
        if (result.status === CalendarRequestStatus.Navigating) {
          ...
          showOverlay(t('home.expanded.chat.navigating', 'Navigating to tile...'), 'info', true);
        } else if (result.status === CalendarRequestStatus.DemoMode) {
          setIsDemoLimited(true);
          setIsNavigating(false);
          showOverlay(
            t('home.expanded.chat.demoMode', 'You are in demo mode with limited features. Sign up to unlock the full experience!'),
            'warning',
          );
        } else {
