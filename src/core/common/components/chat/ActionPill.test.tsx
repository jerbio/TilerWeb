...
} from '@/core/common/components/calendar/CalendarRequestProvider';
import {
	CalendarEntityType,
 ...
 */
const MockCalendarListener: React.FC<{ autoResponse: CalendarRequestResult }> = ({
	autoResponse,
}) => {
...
describe('ActionPill schedule consistency', () => {
	...
	describe('demo mode (anonymous persona)', () => {
		it('shows demo mode overlay after receiving demo_mode result', async () => {
			const user = setupUser();
			const action = createAction({ afterScheduleId: 'schedule-v2' });
			renderActionPillWithAutoResponse(action, {
				status: CalendarRequestStatus.DemoMode,
				entityId: 'entity-abc',
			});
		...
		it('navigating result clears demo limited state', async () => {
			...
			const { unmount } = renderActionPillWithAutoResponse(action, {
				status: CalendarRequestStatus.DemoMode,
				entityId: 'entity-abc',
			});
