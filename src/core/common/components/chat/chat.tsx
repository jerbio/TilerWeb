...
import GateResolutionPrompt, {
	GateResolutionData,
} from '@/core/common/components/chat/GateResolutionPrompt';
import IntegrationAuthPrompt, {
	IntegrationAuthData,
} from '@/core/common/components/chat/IntegrationAuthPrompt';
...
import { deriveDeepThinkingFromMessages } from '@/core/common/components/chat/deriveDeepThinking';
import {
	getDeepThinkingForSession,
	setDeepThinkingForSession,
} from '@/core/storage/chatSessionDeepThinking';
...
const Chat: React.FC<ChatProps> = ({ onClose }) => {
	...
	const [variantSelection, setVariantSelection] = useState<{ vibeRequestId: string } | null>(
		null
	);
	...
	// M10.5 — gate-pending chips (subject disambiguation, artifact selection, integration auth).
	const [gateResolution, setGateResolution] = useState<{
		vibeRequestId: string;
	...
	// Unified Research Path M6.2/M6.5 — DeepThinking toggle, persisted per chat session.
	// Initial value comes from localStorage keyed by the active chat sessionId so the
	// toggle survives chat unmount/remount and switching between sessions. When the
	// sessionId changes we re-load the value for that session.
	const [deepThinking, setDeepThinkingState] = useState<boolean>(() =>
		getDeepThinkingForSession(sessionId)
	);
	const setDeepThinking = useCallback(
		(next: boolean) => {
			setDeepThinkingState(next);
			setDeepThinkingForSession(sessionId, next);
		},
		[sessionId]
	);
	// Re-sync local toggle when the active chat session changes.
	useEffect(() => {
		setDeepThinkingState(getDeepThinkingForSession(sessionId));
	}, [sessionId]);
	...
	const formatWebSocketStatus = (status: string): string => {
		const statusMap: Record<string, string[]> = {
			...
			research_fork_variant_ready: [
				t('home.expanded.chat.wsStatus.researchForkVariantReady'),
			],
			research_fork_awaiting_selection: [
				t('home.expanded.chat.wsStatus.researchForkAwaitingSelection'),
			],
			research_step_awaiting_clarification: [
				t(
					'home.expanded.chat.wsStatus.researchAwaitingClarification',
					'Waiting for your input...'
				),
			],
			research_step_clarification_received: [
				t(
					'home.expanded.chat.wsStatus.researchClarificationReceived',
					'Resuming with your input...'
				),
			],
			research_awaiting_gate_resolution: [
				t(
					'home.expanded.chat.wsStatus.researchAwaitingGateResolution',
					'Waiting for your selection...'
				),
			],
			research_awaiting_integration_auth: [
				t(
					'home.expanded.chat.wsStatus.researchAwaitingIntegrationAuth',
		...
		webSocketCommunication.current.subscribeToSocketDataReceipt((data: unknown) => {
			...
			if (
				...
				if (
					...
					typeof research.totalSteps === 'number'
				) {
					setResearchProgress({
				...
				if (rawStatus === 'research_fork_awaiting_selection') {
					...
					if (sessionId && gateId) {
						...
						setGateResolution({
							...
							gate: {
								...
								stepDescription: research?.stepDescription as string | undefined,
								resultSummary: research?.resultSummary as string | undefined,
					...
					if (sessionId && gateId && integrationKey) {
						setIntegrationAuth({
							...
							gate: {
								gateId,
								integrationKey,
								oauthInitiationUrl: research?.oauthInitiationUrl as
									| string
									| undefined,
								stepDescription: research?.stepDescription as string | undefined,
							},
					...
					rawStatus === 'research_plan_completed' ||
					rawStatus === 'research_fork_resolved'
				) {
	...
	const loadInitialChatMessages = useCallback(
		async (sid?: string) => {
			...
			try {
				...
				shouldAutoScrollToBottomRef.current = true;
				if (sid) {
					const derived = deriveDeepThinkingFromMessages(
						sortedMessages,
						getDeepThinkingForSession(sid)
					);
					setDeepThinkingState(derived);
					setDeepThinkingForSession(sid, derived);
				}
			} catch (err) {
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			...
			const response = await chatService.sendMessage(
				...
				'', // requestId — not threaded from composer
				'', // actionId — not threaded from composer
				deepThinking // M6.2: forwards toggle as `DeepThinking` body field
			);
			if (
	...
	return (
		<ChatWrapper>
			<ChatContainer>
				...
				<div>
					...
					{gateResolution && (
						<GateResolutionPrompt
							vibeRequestId={gateResolution.vibeRequestId}
							gate={gateResolution.gate}
							onResolved={() => setGateResolution(null)}
						/>
					)}
					{integrationAuth && (
						<IntegrationAuthPrompt
							vibeRequestId={integrationAuth.vibeRequestId}
							gate={integrationAuth.gate}
							onResolved={() => setIntegrationAuth(null)}
						/>
					)}
				...
				<DeepThinkingBanner visible={deepThinking} />
				<div
					style={{
						display: 'flex',
						alignItems: 'center',
