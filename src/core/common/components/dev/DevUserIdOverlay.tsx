...
const DevUserIdOverlay: React.FC<DevUserIdOverlayProps> = ({ isVisible, onClose }) => {
	...
	const handleReset = () => {
		...
		setFeedback({
			type: 'success',
			message: 'Custom User ID cleared. Using default behavior.',
		});
	};
	...
	return (
		<Backdrop onClick={onClose}>
			<OverlayCard onClick={(e) => e.stopPropagation()}>
				...
				<WarningBanner>
					⚠️ Development Mode Only - This tool overrides the normal persona user creation
					flow
				</WarningBanner>
				...
				<Section>
					...
					<Description>
						Enter a custom User ID to test specific user requests. When set, all persona
						creations will use this ID instead of creating a new anonymous user.
					</Description>
