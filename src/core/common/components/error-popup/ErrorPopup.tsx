			...
			if (response?.Error?.Code === '0') {
				...
				setEmailError(
					(response?.Error?.Message as string) ||
						t('home.expanded.chat.errorPopup.errors.submitFailed')
				);
			}
		} catch (error) {
		...
		<Overlay onClick={onClose}>
			<PopupContainer onClick={(e) => e.stopPropagation()}>
				...
				<IllustrationContainer>
					<img
						src={FetchingChatInterface}
						alt={t('home.expanded.chat.errorPopup.altText')}
					/>
				</IllustrationContainer>
						...
						<>
							...
							<EmailInputContainer>
								<EmailInputField
									type="email"
									placeholder={t(
										'home.expanded.chat.errorPopup.emailPlaceholder'
									)}
									value={email}
									onChange={(e) => {
							...
							>
								{isSubmitting
									? t('home.expanded.chat.errorPopup.submitting')
									: email.trim()
										? t('home.expanded.chat.errorPopup.submitEmail')
										: t('home.expanded.chat.errorPopup.joinWaitlist')}
								{!isSubmitting && <ArrowRight size={16} />}
							</Button>
							<WaitlistSubtext>
								{t('home.expanded.chat.errorPopup.subtext')}
							</WaitlistSubtext>
						</>
					)}
					{onRedirect && (
...
const EmailInputField = styled.input<{ $hasError: boolean }>`
	...
	border: 1px solid
		${(props) => (props.$hasError ? palette.colors.error[500] : palette.colors.gray[700])};
	...
	&:focus {
		outline: none;
		border-color: ${(props) =>
			props.$hasError ? palette.colors.error[500] : palette.colors.brand[500]};
	}
