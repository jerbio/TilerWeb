 ...
 *
 * Contract (verified against TilerFront's `IntegrationsController`): the
 * client navigates the browser (top-level GET with session cookies) to
 * `GET <base>api/Integrations/connect?provider=google&redirectTarget=<own-
 * origin https URL>` and the server 302-redirects to the provider consent
 * screen.
 */
describe('buildOauthStartUrl', () => {
	...
	it('targets the server-owned calendar-connect start endpoint', () => {
		// `api/Integrations` without `/connect` is the integrations list endpoint.
		expect(OAUTH_START_PATH).toBe('api/Integrations/connect');
	});
	...
	it('does not depend on a trailing slash in the base URL', () => {
		const expected =
			'https://api.tiler.test/api/Integrations/connect?provider=google&redirectTarget=' +
